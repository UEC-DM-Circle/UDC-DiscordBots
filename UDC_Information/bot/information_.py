...
async def ready(num: int):
    ...
    for a in articles:
        ...
        if (
            ("入賞数ランキング" in title)
            or ("結果" in title)
            or (("公開" in title) and ("『" in title) and ("プレミア公開" not in title))
        ):
...
async def result_check(response):
    ...
    imgs = [
        img.find("img").get("src") for img in result_imgs if img.find("img") is not None
    ]
...
async def newcard_check(response):
    ...
    newcard_img = [
        img.find("img").get("src") for img in newcard if img.find("img") is not None
    ]
    if newcard_img == []:
        ...
        if new_article not in latest_articles:
            try:
                ...
                if "入賞数ランキング" in article_title:
                    ...
                    ("公開" in article_title)
                    and ("『" in article_title)
                    and ("プレミア公開" not in article_title)
                ):
